...
abstract class BaseManyToMany<T1, T2> {
  ...
  Future<void> createLinkWithEntity(JsonMap entityData, int id1, int id2) async {
    await database.transaction((txn) async {
      final int newEntityId = await txn.insert(
        tableName,
        entityData,
        conflictAlgorithm: ConflictAlgorithm.replace,
      );
...
class ManyToManyBuilder {
  ...
  final String table1Name;
  final String table2Name;
  late DatabaseTableBuilder databaseTableBuilder;
  ...
  ManyToManyBuilder(
      ...
      required this.table1Name,
      required this.table2Name}) {
    databaseTableBuilder = DatabaseTableBuilder(onCreate: '''
